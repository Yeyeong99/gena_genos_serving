...
export async function POST(request: Request) {
  ...
  try {
    ...
    const response = await axios.post(
      `${LOCAL_BACKEND_URL.replace(/\/$/, "")}/api/document-translation/translate/revise`,
      payload,
      {
        validateStatus: () => true,
      },
    ...
    if (response.status >= 400) {
      return NextResponse.json(
        { message: response.data?.message ?? "번역 수정 요청에 실패했습니다." },
    ...
    const message =
      error instanceof Error ? error.message : "번역 수정 요청 중 알 수 없는 오류가 발생했습니다.";
