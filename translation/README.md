...
| `name` | `string` | 권장 | 확장자 판별에 사용하는 원본 파일명. `metadata.file_name` 으로도 전달 가능 |
| `type` | `string` | 권장 | `pptx`, `docx`, `xlsx`, `pdf` 등 파일 타입 메타 |
| `metadata.file_name` | `string` | 권장 | `name` 대신 사용할 수 있는 원본 파일명 |
